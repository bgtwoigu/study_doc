...
COMMIT_PATCH_FILENAME = 'release_notes.txt'
...
def rename_patch_name(oldFilePath, newFileName):
    filepath = os.path.dirname(oldFilePath)
    newFilePath = os.path.join(filepath, newFileName)
    #print filepath
    #print newFilePath
    os.rename(oldFilePath, newFilePath)

def save_svn_patch_title(backup_dir, svn_patch_title_list):
    filepath = backup_dir + '/' + COMMIT_PATCH_FILENAME
    fp = open(filepath, 'w')
    for item in svn_patch_title_list:
        itemStr = '\t'.join(item) + '\r\n'
        fp.write(itemStr)
    fp.close()
...
def main():
    ...
    end_commit_id = '11ab8650411473821f747c2c1964bdd45e19062d'
    ...
    for item in git_log_infoList[::-1]:
        ...
        author = info[1].strip()
        date = info[2].strip()
        commit_info = info[3].strip()
        title_format = info[4].strip()
        svn_id = info[5].strip().split('@')[-1].split(' ')[0]

        patch_name = '_'.join([svn_id, author, date, title_format]) + '.patch'
        #print patch_name
        author = info[1].strip() + ':'
        #print info
        ...
        #new features
        rename_patch_name(filepath, patch_name)
        
        svn_patch_title.append([svn_id, patch_name, commit_info])
    ...
    save_svn_patch_title(backup_dir, svn_patch_title)
